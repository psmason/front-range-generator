...
def printRidges(img, ridges, rgb):
    ...
    for ridge in ridges:
        rgb = COLOR_FNS[COLOR_FN](rgb) # can also try increaseSaturation(rgb)
...
def generateSky(img, skyline, startColor, endColor):
    ...
    for x,y in skyline.iteritems():
        for pos in range(0, y+5):            
            ...
            weightedColor = tuple([int(t[0] + t[1]) for t in zip(weightedStart, weightedEnd)])
